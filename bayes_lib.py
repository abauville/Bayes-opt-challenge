...
def get_x_new(aquisition_func, model, device='cpu'):
    ...
    # note: gen_candidates_scipy is slightly faster than _torch (with default params) at the resolution of our experiment
...
def run_experiment(gt_func, n_iter=1, n_train_ini=1, print_period=1):
    ...
    train_x = torch.rand(6*n_train_ini, dtype=torch.float64).reshape(-1,6).to(device)
    # note: train_x, train_y must be float64, float32 often leads to negative 
    # variance and not positive definite matrices (i.e. warnings and errors) 
...
def baseline_model(gt_func, n_iter, n):
    ...
    results[0] = gt_func(torch.rand(n*6).reshape(-1,6)).max()

    for i in range(1, n_iter):
        results[i] = max(results[i-1], gt_func(torch.rand(n*6).reshape(-1,6)).max())
    return results